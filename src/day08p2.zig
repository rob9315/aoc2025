...
pub fn main() !void {
    ...
    var distances: std.ArrayList(Dist) = .empty;
    for (coordinates.items, 0..) |coord, i| {
        for (coordinates.items[0..i], 0..) |coord2, j| {
            ...
            (try distances.addOne(gpa)).* = distance;
        }
    }

    std.sort.block(Dist, distances.items, {}, Dist.asc);
    ...
    for (distances.items) |dist| {
        ...
        if (coord_group[0]) |_| {
            if (try find_in_ref_loop(coord_group, 0, 0) == coord_group.len) {
                print("{}\n", .{@as(u64, coordinates.items[i_1].x) * @as(u64, coordinates.items[i_2].x)});
                return;
            }
        }
        // print("{any}\n", .{coord_group});
...
const Dist = struct {
    ...
    i_1: usize,
    i_2: usize,
    fn asc(_: void, d1: Dist, d2: Dist) bool {
